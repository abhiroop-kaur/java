...
public class TicTacToe extends Canvas {
    ...
    public static int[][] createBoard(int row, int column){
        int[][] board;					//Create variable
        board = new int[row][column];		//Create 2D array (filled with 0’s)
        return board;
    }
    //rowsIn
    public static int rowsIn(int[][] board ){
        int lengthRow;
        lengthRow = board.length; //finding length of rows
        return lengthRow;
    }
    //columnsIn
    public static int columnsIn(int[][] board ){
        int lengthColumns;
        lengthColumns=board[0].length; //finding length of columns
    ...
    public static boolean canPlay(int[][] board, int row, int column ){
        boolean bool;
        if(board[row][column] == 0){ //checking for empty space to play in
            bool=true;
        }else {
            bool =false;
        }
        return bool; // returning the boolean after checking
    }
    //play
    public static void play(int[][] board,int row, int column,int piece ) {
        board[row][column]=piece; //placing the piece in the given index
    }

    //full
    public static boolean full(int[][] board ){
        for(int i=0;i<rowsIn(board);i++) { //looping through rows
            for (int j = 0; j <columnsIn(board) ; j++) { //looping through columns
                if (board[i][j] == TicTacToe.EMPTY) { // checking if the board is full
                    return false;
                }
            }
        }return true;
    ...
    public static boolean winInRow(int[][] board,int row, int piece ){
        for(int i =0;i<rowsIn(board);i++){ //looping through rows
            if(board[row][i]==piece){
                if(board[row][i+1]==piece){ // checking if player wins in any row
                    if(board[row][i+2]==piece){
                         return true;
                    }
                }
            }
    ...
    public static boolean winInColumn(int[][] board,int column, int piece ){
        for(int i =0;i<columnsIn(board);i++){ //looping through columns
            if(board[i][column]==piece){
                if(board[i+1][column]==piece){// checking if player wins in any columns
                    if(board[i+2][column]==piece){
                        return true;
                    }
                }
            }
    ...
    public static boolean winInDiagonalBS(int[][] board,int piece){
        int x=0;
        int count=0;
        for (int i=0 ;i<board.length;i++){// checking if player wins in  backslash diagonals
            if(board[i][x]==piece){
             count++;
             x++;
            }
        }if(count>=3){ //checking for the count of same elements in diagonal
            return true;
        }else{
            return false;
        }
    ...
    public static boolean winInDiagonalFS(int[][] board,int piece){
        ...
        int count=0;
        for (int i=0 ;i<board.length;i++){// checking if player wins in front slash diagonals
            if(board[i][x]==piece){
                count++;
                x--;
            }
        ...
        for (int i=0;i<rowsIn(board);i++){//looping through rows
            for(int j=0 ;j<columnsIn(board);j++){//looping through columns
              if(canPlay(board,i,j)){
                  ...
                  if(won(board,piece)){ //checking if player wins
                      board[i][j]=0;
                      int[] array1={i,j};
                      return array1;
